...
const char* aff_path = "/usr/share/hunspell/en_GB.aff";
const char* dic_path = "/usr/share/hunspell/en_GB.dic";
...
static int check_word(struct state* st) {
    ...
    if (st->wordlen == SMALLBUF) return 1;
...
static void process(struct state* st, const char* buf, size_t len) {
        ...
        switch (c) {
            ...
            case 'a' ... 'z':
                switch (st->state) {
                    ...
                    case Escape:
                        handle_escape(st, c);
                        if (c == 'm') remember_escape(st);
                        st->state = Noise;
                        continue;
                    default:
                        if (last < buf) write(1, last, buf - last);
                        st->state = Word;
                        st->wordlen = 1;
                        st->word[0] = c;
                        continue;
            ...
            // Special case
            case '\'':
                if (st->state == Word && len > 1 && isalpha(buf[1]) &&
                    st->wordlen < SMALLBUF) {
                    st->word[st->wordlen++] = c;
                    continue;
                }
    ...
    if (st->state != Word && last < buf) write(1, last, buf - last);
}
...
int main(int argc, char** argv) {
    ...
    if (isatty(0)) exec_less(argv);

    Hunhandle* hunhandle = Hunspell_create(aff_path, dic_path);

    if (!hunhandle || pipe(fd)) exec_less(argv);
    ...
    if (state.state == Word) check_and_print(&state);
